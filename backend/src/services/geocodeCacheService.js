...
const NEAREST_CACHE_MAX_METERS = 25;
...
async function getAddressAsync(lat, lon) {
  ...
  const lookupPromise = (async () => {
    ...
    try {
      ...
      if (!address) {
        return await getNearestCachedAddressAsync(lat, lon);
      }
...
async function getNearestCachedAddressAsync(lat, lon, maxMeters = NEAREST_CACHE_MAX_METERS) {
  await ensureCacheLoaded();

  const targetLat = Number(lat);
  const targetLon = Number(lon);
  const limit = Number(maxMeters);
  if (!Number.isFinite(targetLat) || !Number.isFinite(targetLon) || !Number.isFinite(limit) || limit <= 0) {
    return null;
  }

  let best = null;
  for (const [key, item] of cacheMap.entries()) {
    const [cachedLat, cachedLon] = String(key || '').split(',').map(Number);
    if (!Number.isFinite(cachedLat) || !Number.isFinite(cachedLon)) {
      continue;
    }

    const address = normalizeAddressText(item?.address || null);
    if (!address) {
      continue;
    }

    const meters = distanceMeters(targetLat, targetLon, cachedLat, cachedLon);
    if (meters <= limit && (!best || meters < best.meters)) {
      best = { key, address, meters };
    }
  }
  ...
  await touchCacheEntry(best.key);
  return best.address;
}
